...
import { ATATUS_REQ_FIELDS } from '../src/constants';
...
describe('helpers', function() {
  describe('handleError', function() {
    ...
    it('should default prefix to "AtatusSourceMapPlugin"', function() {
      const result = helpers.handleError(new Error('required field missing'));
      expect(result.length).toEqual(1);
      expect(result[0]).toInclude({
        message: 'AtatusSourceMapPlugin: required field missing'
      });
    });
  ...
  describe('validateOptions', function() {
    it('should return null if all required options are supplied', function() {
      const options = {
        Authorization: 'aaabbbccc000111',
        version: 'latest',
        url: 'https://my.cdn.net/assets'
      };
      const result = helpers.validateOptions(options);
      expect(result).toBe(null); // eslint-disable-line no-unused-expressions
    });

    it('should return an error if Authorization is not supplied', function() {
      const options = {
        version: 'latest',
        url: 'https://my.cdn.net/assets'
      };
      ...
      expect(result[0]).toBeA(Error)
        .toInclude({ message: 'required field, \'Authorization\', is missing.' });
    });
    ...
    it('should return an error if url is not supplied', function() {
      const options = {
        Authorization: 'aaabbbccc000111',
        version: 'latest'
      };
      ...
      expect(result.length).toBe(1);
      expect(result[0]).toBeA(Error)
        .toInclude({ message: 'required field, \'url\', is missing.' });
    });
    ...
    it('should handle multiple missing required options', function() {
      ...
      const result = helpers.validateOptions(options);
      expect(result).toBeA(Array);
      expect(result.length).toBe(ATATUS_REQ_FIELDS.length);
    });
